...
import { CustomerType, useTrackedOrderStore } from '../store/orderStore';
...
const CustomerSearch = () => {
	...
	return (
		...
		>
			<div className="bg-white w-full px-4 pt-6 min-[448px]:pt-4 pb-3 shadow-md absolute top-0 left-0">
				...
				{list?.map((customer)=> {
					return (
						<div
							...
							className={`w-full p-4 flex gap-4 hover:bg-white items-center ease-in-out border border-transparent ${
								colActive?.id === customer.id ? 'col-active' : ''
							}`}
							onClick={() => selectCustomer(customer)}
						>
							<div
								className={`mb-4 rounded-full ${customer.img} text-white w-10 h-10 font-medium text-sm flex justify-center items-center`}
							>
								{customer.name[0]}
							</div>
							<div>
			...
			<button className="min-[448px]:hidden absolute text-theme-blue p-4 w-full h-16 bottom-0 bg-white text-center font-medium text-sm shadow-[0_-2px_4px_3px_rgba(108,114,124,0.1)]" onClick={()=>confirmPanel()}>
				Pilih Pelanggan
			</button>
		</div>
	);
